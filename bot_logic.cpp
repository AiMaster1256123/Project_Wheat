...
static bool WaitForValidFile(const std::string& path, int timeoutMs = 180) {
    const int pollMs = 20;
    for (int elapsed = 0; elapsed <= timeoutMs; elapsed += pollMs) {
        if (IsFileValid(path)) return true;
...
std::string ResolveTemplatePath(const std::string& templatePath) {
    ...
    std::error_code ec;
    fs::path requested(templatePath);
    if (requested.is_absolute()) {
        return fs::exists(requested, ec) ? requested.lexically_normal().string() : templatePath;
    }

    auto findFromBase = [&](fs::path base) -> std::string {
        for (int depth = 0; depth < 6 && !base.empty(); ++depth) {
            fs::path candidate = (base / requested).lexically_normal();
            ec.clear();
...
namespace {
    constexpr size_t kMaxCachedTemplateImages = 96;

    struct CachedTemplateImage {
        cv::Mat image;
        std::filesystem::file_time_type lastWriteTime{};
        uintmax_t fileSize = 0;
    };

    std::mutex g_TemplateCacheMutex;
    std::unordered_map<std::string, CachedTemplateImage> g_TemplateImageCache;

    cv::Mat LoadTemplateCached(const std::string& templatePath, int flags) {
        namespace fs = std::filesystem;

        std::string resolvedPath = ResolveTemplatePath(templatePath);
        fs::path path(resolvedPath);
        std::error_code ec;
        ...
        if (exists) {
            ec.clear();
            lastWriteTime = fs::last_write_time(path, ec);
            if (ec) lastWriteTime = fs::file_time_type{};
        ...
        std::string cacheKey = resolvedPath + "|" + std::to_string(flags);
        {
            std::lock_guard<std::mutex> lock(g_TemplateCacheMutex);
            auto it = g_TemplateImageCache.find(cacheKey);
            if (it != g_TemplateImageCache.end() &&
        ...
        cv::Mat loaded = cv::imread(resolvedPath, flags);
        if (loaded.empty()) return loaded;

        {
            std::lock_guard<std::mutex> lock(g_TemplateCacheMutex);
            if (g_TemplateImageCache.size() >= kMaxCachedTemplateImages) {
                g_TemplateImageCache.erase(g_TemplateImageCache.begin());
            }
            g_TemplateImageCache[cacheKey] = CachedTemplateImage{ loaded, lastWriteTime, fileSize };
        }
...
MatchResult FindImage(const cv::Mat& screen, const std::string& templatePath, float threshold, bool useGrayscale, float roiPercent, bool useMargins) {
    ...
    cv::Mat templ = LoadTemplateCached(templatePath, flags);
...
std::vector<MatchResult> FindAllImages(const cv::Mat& screen, const std::string& templatePath, float threshold, int minDist, bool useMargins) {
    ...
    cv::Mat templ = LoadTemplateCached(templatePath, cv::IMREAD_COLOR);
